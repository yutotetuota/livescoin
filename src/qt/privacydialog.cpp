...
#include "zlivescoincontroldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zLIVES ought to be enough for anybody." - Bill Gates, 2017
    ui->zLIVESpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzLIVESSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzLIVES->setEnabled(false);
        ui->pushButtonMintzLIVES->setToolTip(tr("zLIVES is currently disabled due to maintenance."));

        ui->pushButtonSpendzLIVES->setEnabled(false);
        ui->pushButtonSpendzLIVES->setToolTip(tr("zLIVES is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zLIVESpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzLIVES_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLIVES is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zLIVES...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zLIVES in ") +
...
void PrivacyDialog::on_pushButtonSpendzLIVES_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zLIVES is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zLIVES
        sendzLIVES();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zLIVES
    sendzLIVES();
}

void PrivacyDialog::on_pushButtonZLIVEsCOINControl_clicked()
{
    ZLIVEsCOINControlDialog* zLIVEsCOINControl = new ZLIVEsCOINControlDialog(this);
    zLIVEsCOINControl->setModel(walletModel);
    zLIVEsCOINControl->exec();
}

void PrivacyDialog::setZLIVEsCOINControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzLIVEsCOINSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzLIVES()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid LIVEsCOIN Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zLIVESpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zLIVES is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " LIVES </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zLIVES</b>";
    ...
    // use mints from zLIVEsCOIN selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZLIVEsCOINControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZLIVEsCOINControlDialog::GetSelectedMints();
    }
    ...
    // Clear zlivescoin selector in case it was used
    ZLIVEsCOINControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zLIVEsCOIN Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " LIVEsCOIN, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zLIVEsCOIN Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zLIVES </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zLIVES "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zLIVES "));
    ui->labelzLIVESAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzLIVESSyncStatus->setVisible(fShow);
}
