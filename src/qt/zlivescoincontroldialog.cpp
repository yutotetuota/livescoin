...
#include "ui_zlivescoincontroldialog.h"
...
std::list<std::string> ZLIVEsCOINControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZLIVEsCOINControlDialog::listMints;

ZLIVEsCOINControlDialog::ZLIVEsCOINControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZLIVEsCOINControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZLIVEsCOINControlDialog::~ZLIVEsCOINControlDialog()
{
    delete ui;
}

void ZLIVEsCOINControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZLIVEsCOINControlDialog::updateList()
{
...
void ZLIVEsCOINControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZLIVEsCOINControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZLIVEsCOIN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZLIVEsCOINControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZLIVEsCOINControlDialog::GetSelectedMints()
{
...
void ZLIVEsCOINControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
