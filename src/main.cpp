...
#if defined(NDEBUG)
#error "LIVEsCOIN cannot be compiled without assertions."
#endif
...
static unsigned int nStakeMinAgeV1 = 12 * 60 * 60; // 12 hours
static unsigned int nStakeMinAgeV2 = 24 * 60 * 60; // 24 hours after block 86400
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zLIVEsCOIN spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zLIVEsCOIN spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zLIVEsCOIN spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if(IsTreasuryBlock(nHeight)) {
        ...
        if (nHeight == 0) {
            nSubsidy = 7000000000 * COIN; //Premine
        } else if (nHeight <= 10079 && nHeight >= 1) {
            nSubsidy = 100 * COIN;
        } else if (nHeight <= 43199 && nHeight >= 10080) {
            nSubsidy = 2000 * COIN;
            ...
            nSubsidy = 1100 * COIN;
        } else if (nHeight <= 1814399 && nHeight >= 1728000) {
            nSubsidy = 1000 * COIN;
        } else if (nHeight <= 1900799 && nHeight >= 1814400) {
            nSubsidy = 950 * COIN;
        } else if (nHeight <= 1987199 && nHeight >= 1900800) {
            nSubsidy = 900 * COIN;
        } else if (nHeight <= 2073599 && nHeight >= 1987200) {
            nSubsidy = 850 * COIN;
        } else if (nHeight <= 2159999 && nHeight >= 2073600) {
            nSubsidy = 800 * COIN;
        } else if (nHeight <= 2246399 && nHeight >= 2160000) {
            nSubsidy = 750 * COIN;
        } else if (nHeight <= 2332799 && nHeight >= 2246400) {
            nSubsidy = 700 * COIN;
        } else if (nHeight <= 2419199 && nHeight >= 2332800) {
            nSubsidy = 650 * COIN;
        } else if (nHeight <= 2505599 && nHeight >= 2419200) {
            nSubsidy = 600 * COIN;
        } else if (nHeight <= 2591999 && nHeight >= 2505600) {
            nSubsidy = 550 * COIN;
        } else {
            nSubsidy = 500 * COIN;
        }
...
//Treasury blocks
int nStartTreasuryBlock = 9999999; //Invalid
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from LIVEsCOIN
...
{
    RenameThread("livescoin-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZLIVESMinted()
{
...
void RecalculateZLIVESSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zLIVES supply
...
bool RecalculateLIVESSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zLIVEsCOIN with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zLIVEsCOINSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert LIVES to zLIVES
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zLIVEsCOIN spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zLIVEsCOIN serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zLIVEsCOIN mints and %d zLIVEsCOIN spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // LIVEsCOIN: We use certain sporks during IBD, so check to see if they are
